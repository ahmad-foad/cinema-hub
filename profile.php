...
<div class="profile-section">
    ...
    echo <<<TEXT
        ...
        <h1 id="full-user-name">$firstName $lastName</h1>
        <h6 id="username">@$userName</h6>
    TEXT;
    ?>
    <form action="/server/logout.php" method="POST">
        <button type="submit" class="logout-button" id="logout-button">Logout
...
<script>
    ...
    $userId = $_SESSION['user_id'];
    $pdo = new PDO('sqlite:database/cinema-hub.db');
    $userHistoryQuery = $pdo->query("SELECT * FROM history WHERE user_id = '$userId'");
    $userHistory = $userHistoryQuery->fetchAll(PDO::FETCH_ASSOC);
    echo 'window.USER_HISTORY = ' . json_encode($userHistory) . ';';
    ?>
</script>
